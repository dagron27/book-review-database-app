...
Early planning notes from development. The "Create the Database" / "Run
the webserver" steps below predate this repo's `src/`/`data/`
reorganization and are superseded by the current README's Environment
Setup section -- kept here as a historical record of the schema design,
not as current setup instructions.
